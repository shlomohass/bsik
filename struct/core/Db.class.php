...
// SEND_DB_ERRORS LOG_DB_ERRORS SEND_ERRORS_TO LOG_DB_TO_TABLE
if (!defined('SEND_DB_ERRORS')) define('SEND_DB_ERRORS', false);
if (!defined('SEND_ERRORS_TO')) define('SEND_ERRORS_TO', "");
if (!defined('LOG_DB_ERRORS')) define('LOG_DB_ERRORS', false);
if (!defined('LOG_DB_TO_TABLE')) define('LOG_DB_TO_TABLE', "");
...
//QUERY EXECUTION:
    ...
    /**
     * Perform queries will log execution time and report back in sec
     * All following functions run through this function
     * All data run through this function is automatically sanitized using the filter function
     *
     * @access private
     ...
     *
     */
    public function exec_query_timed($query, &$time) {
        $test_micro = microtime(true);
        $results = $this->link->query( $query );
        $time = number_format((microtime(true) - $test_micro),5);
        Trace::add_query_trace($query, (microtime(true) - $test_micro), $this->link->info);
        return $results;
    }
    /**
